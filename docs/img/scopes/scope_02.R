...
grid$total <- grid$inner - grid$outer

grid$index <- round(1 + (grain - 1) * normalise(grid$total))

palette <- scico::scico(n = grain, palette = "lajolla")
grid$color <- palette[grid$index]

rast <- as.raster(grid, value = color)

png(
  filename = "~/Desktop/scope_02.png",
  width = grain,
  height = grain,
)
